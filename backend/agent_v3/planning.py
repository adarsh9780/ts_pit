...
def _step_is_schema_grounding_text(text: str) -> bool:
    ...
    return (
        ...
        or "logical" in lowered and "column" in lowered
    )
...
def _ensure_schema_grounding_step(
    ...
    existing_has_schema = any(_step_has_schema_grounding(step) for step in existing_steps)
...
def _merge_plan(state: AgentV3State, plan: Plan) -> tuple[list[StepState], list[StepState]]:
...
def planner(state: AgentV3State, config: RunnableConfig) -> dict[str, Any]:
    ...
    pending = [_step_snapshot(s) for s in state.steps if s.status in {"pending", "running"}]
    ...
    if plan.requires_execution and plan.plan_action in {"append", "replace"} and not plan.steps:
        plan = plan.model_copy(
            update={
                "steps": [
                    PlannerStep(
