...
from ts_pit.database import get_db_connection, remap_row
from ts_pit.db import get_engine
from ts_pit.config import get_config
from ts_pit.alert_analysis import (
    analyze_alert_non_persisting,
    get_current_alert_news_non_persisting,
)
from ts_pit.reporting import generate_alert_report_html, sanitize_session_id
from ts_pit.services.db_helpers import (
    get_alert_id_candidates,
    get_table,
    probe_alert_id_values,
)
    ...
    try:
        ...
        for value in probe_alert_id_values(alert_id):
            for id_col in get_alert_id_candidates(table_name):
                stmt = (
                    select(*[cast(c, Text).label(c.name) for c in alerts.columns])
                    .where(cast(alerts.c[id_col], Text) == str(value))
                    .limit(1)
                )
                with engine.connect() as conn:
...
def get_alerts_by_ticker(ticker: str) -> str:
    ...
    try:
        ...
        if not results:
            return _ok(
                [], message=f"No alerts found for ticker {ticker}.", ticker=ticker
            )
...
def get_price_history(ticker: str, period: str = "1mo") -> str:
    ...
    """
    from ts_pit.prices import fetch_and_cache_prices
    ...
    try:
        ...
        return _error(
            f"Error fetching prices: {str(e)}", code="PRICE_ERROR", ticker=ticker
        )
...
def get_current_alert_news(alert_id: str, limit: int = 50) -> str:
    ...
    try:
        ...
        if not result.get("ok"):
            return _error(
                result.get("error", "Failed to fetch alert news"),
                code="ALERT_NEWS_ERROR",
            )
        return _ok(
            result["articles"],
            alert_id=alert_id,
        ...
        return _error(
            f"Error fetching current alert news: {str(e)}", code="ALERT_NEWS_ERROR"
        )
    finally:
        conn.close()
