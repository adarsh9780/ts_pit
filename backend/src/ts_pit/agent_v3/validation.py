...
from ts_pit.agent_v3.responding import completed_step_payloads, failed_step_payloads
from ts_pit.agent_v3.state import AgentV3State, AnswerFeedback
...
def _append_fallback_note(answer: str, feedback: AnswerFeedback) -> str:
    ...
    reason = (
        feedback.reason
        or "Response quality checks could not produce a stronger answer."
    )
    return (
...
def _deterministic_feedback(
    state: AgentV3State, draft_answer: str
) -> AnswerFeedback | None:
...
def _finalize_accept(
    state: AgentV3State, answer_text: str, feedback: AnswerFeedback
) -> dict[str, Any]:
...
def answer_validator_node(
    state: AgentV3State, config: RunnableConfig
) -> dict[str, Any]:
    _ = config
