...
TABLE_LOGICAL_ALIAS_OVERRIDES: dict[str, dict[str, str]] = {
    "alerts": {
        "alert_id": "id",
...
def _table_logical_to_physical_column_map(table_key: str) -> dict[str, str]:
    ...
    for logical_name, physical_name in cols.items():
        ...
        mappings[_norm_identifier(logical)] = physical
        logical_to_physical[logical] = physical

    for alias, logical_key in TABLE_LOGICAL_ALIAS_OVERRIDES.get(table_key, {}).items():
        physical = logical_to_physical.get(logical_key)
        if not physical:
            continue
        mappings[_norm_identifier(alias)] = physical
    return mappings
    ...
    try:
        ...
        if "no such column" in msg.lower():
            try:
                cfg = get_config()
                hint = (
                    " Hint: write SQL with logical column names (for example: id, ticker, alert_date). "
                    "This tool maps logical names to physical DB columns from config.yaml automatically. "
                    "Read artifacts/DB_SCHEMA_REFERENCE.yaml for logical-to-db mappings if needed."
                )
            except Exception:
                hint = (
                    " Hint: write SQL using logical schema names and let execute_sql map them "
                    "to DB columns."
                )
        ...
        return _ok(
            {
                ...
                "primary_alert_id": analysis.get("primary_alert_id"),
