...
def main():
    ...
    if not existing_cols:
        print("No legacy analysis columns found. Table created/verified; nothing to backfill.")
        conn.commit()
        conn.close()
        return

    select_cols = ", ".join([quote_ident(alert_id_col)] + [quote_ident(c) for c in existing_cols])
    ...
    for row in rows:
        ...
        generated_at = payload.get("summary_generated_at") or datetime.utcnow().isoformat()

        has_any = any(
            value is not None and str(value).strip() != ""
            for value in [theme, summary, bullish, bearish, neutral, recommendation, reason]
        )
        if not has_any:
            skipped += 1
            continue
